...
这是原 C++/Qt 项目的独立 Python 重写版。当前版本已包含六个页面、正逆运动学、坐标与关节限幅、统一系统状态、模拟/真实 I²C 双后端、安全日志、机械臂降级分拣演示和故障注入入口。

当前实物没有可用传送带。传送带M4、传送带S3和四路推杆统一显示黄色 `WARN`，但不会阻塞M1/M2/M3、旋转舵机和夹爪的核心流程；自动演示会跳过相关步骤，由机械臂直接完成分类放置。核心通信或机械臂节点故障仍显示红色 `ERROR` 并阻塞运动。
...
硬件模式使用 `/dev/i2c-1`、地址 `0x20` 和FPGA v5寄存器协议。运动目标链路已升级为有符号32位脉冲。
